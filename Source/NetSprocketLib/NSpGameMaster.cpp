								...
								NSpProtocolPriv *theProt)
{
	...
	//ƒ	Check the flags.  Create a player on this workstation if necessary
		...
		NMUInt32				inReceivedTime)
{
	...
	if (inMessage->customDataLen == kCustomLocalJoinRequest)		//ƒ	It's a local request
	{
		approved = true;					//ƒ	Always approve the local request
		localRequest = true;
	}
	...
	else if (approved && mJoinRequestHandler != NULL)		//ƒ	Call the custom join handler, if one is installed
	{
		// dair, added NSpJoinResponseMessage support
	...
	//ƒ	Reject them if they gave an incorrect password
	...
	if (approved)
	{
		...
		//ƒ	Add an entry to the playermap list
		NSpPlayerInfo	info;
		...
		{
			mPlayerID = player;
			status = kNMNoError;		//ƒ	Don't send the join approved to a local joiner
		}
		else
		{
				// dair, added NSpJoinResponseMessage support
			...
			//ƒ	Include the old player map for the new player	
			status = SendJoinApproved(commEndpoint, player, inReceivedTime);
		}
		
		if (status == kNMNoError)
		{
			//ƒ	Now notify everyone else that there is a new player
			//ƒ	If this fails, we could get out of sync
			status = NotifyPlayerJoined(&info);
		}
		else 
		{
			RemovePlayer(player, true);		//ƒ	We couldn't send the approval for some reason.  Undo the add.
...
NSpGameMaster::RouteMessage(NSpMessageHeader *inHeader, NMUInt8 *inBody, NSpFlags inFlags)
{
	...
	if (toPlayer == kNSpAllPlayers)			//ƒ	To all
	{
		//ƒ	Send to the local player first, provided he wasn't the sender.
...
#if !big_endian
		//ƒ	Byte-swap the message for sending now, and don't do it on each call
		...
		//ƒ	Now loop through all players...
		iter->Reset();
		while (iter->Next(&theItem))
		{
			thePlayer = (PlayerListItem *)theItem;
			
			if (fromPlayer != thePlayer->id)	//ƒ	Don't send it to the sender
			{
				if (thePlayer->id != mPlayerID)		//ƒ	Don't byte-swap (already done).
					status = thePlayer->endpoint->SendMessage(inHeader, inBody, inFlags, false);
			}			
		}
	}
	else if (toPlayer == kNSpMasterEndpointID)		//ƒ	To the host only
	{
		status = DoSelfSend(inHeader, inBody, inFlags);
	}
	else if (toPlayer == mPlayerID)			//ƒ	To us
	{
		status = DoSelfSend(inHeader, inBody, inFlags);
	}
	else if (toPlayer > kNSpAllPlayers)		//ƒ	To a specific player that is not us
	{
		//ƒ	Find that person
		//ƒ	How can we make this faster?
		iter->Reset();
	...
	else if (toPlayer < kNSpMasterEndpointID)		//ƒ	To a group
	{	
		...
		//ƒ	First find the group in the list
		...
		//ƒ	If we didn't find it, bail
		if (!found)
			return 	(kNSpInvalidGroupIDErr);

		//ƒ	Now we have the group, lets iterate over the players, sending to them
		//ƒ	Iterate through our player list
		...
		//ƒ	Now loop through all players in the group...
		while (playerIterator->Next(&theItem))
		{
			...
			{
				if (thePlayer->id == mPlayerID)		//ƒ	Will need to byte-swap below.
					performSelfSend = true;
				else								//ƒ	Don't byte-swap (already done above).
					status = thePlayer->endpoint->SendMessage(inHeader, inBody, inFlags, false);
		...
		//ƒ	If the local player was in the group, then you have to send the message
...
NSpGameMaster::SendUserMessage(NSpMessageHeader *inMessage, NSpFlags inFlags)
{
	...
	if (inTo == kNSpAllPlayers)			//ƒ	To all
	{
		//ƒ	First give it to ourselves
		if (inFlags & kNSpSendFlag_SelfSend)
			status = DoSelfSend(inMessage, (NMUInt8 *) inData, inFlags);

		//ƒ	Now hand it off to the host to send to everyone
		status = mPlayersEndpoint->SendMessage(inMessage, (NMUInt8 *) inData, inFlags);
	}
	else if (inTo == kNSpMasterEndpointID)		//ƒ	To the host only
	{
		status = mPlayersEndpoint->SendMessage(inMessage, (NMUInt8 *) inData, inFlags);
	}
	else if (inTo == mPlayerID)			//ƒ	To us
	{
		status = DoSelfSend(inMessage, inData, inFlags);
#if !big_endian
		swapBack = false;				//ƒ	Message never got sent & swapped, so don't swap back.
#endif
	}
	else if (inTo > kNSpAllPlayers)		//ƒ	To a specific player
	{
		status = mPlayersEndpoint->SendMessage(inMessage, (NMUInt8 *) inData, inFlags);
	}
	else if (inTo < kNSpMasterEndpointID)		//ƒ	To a group
	{
		//ƒ	If we're a member, send it to ourselves before sending to the host
		groupIter->Reset();
...
NSpGameMaster::SendTo(NSpPlayerID inTo, NMSInt32 inWhat, void *inData, NMUInt32 inLen, NSpFlags inFlags)
{
	...
	//Ä	Don't allow anyone to send a user message on a headless server
	//Ä	Fixes bug 1366502
	if (bHeadlessServer)
		return (kNSpSendFailedErr);
...
NSpGameMaster::SendJoinApproved(CEndpoint *inEndpoint, NSpPlayerID inID, NMUInt32 inReceivedTime)
{
	...
	if (status == kNMNoError)
	{
		...
		//ƒ	Free the memory from the buffer.  Free the mem alloced by MakeJoinApprovedMessage
		...
		NMUInt32					inReceivedTime)
{
	...
	//ƒ	Now we can construct the size of the entire message
	...
	*theMessage = (TJoinApprovedMessagePrivate *) InterruptSafe_alloc(messageSize);
	...
	//ƒ	Now fill in the player stuff and the group stuff
	...
	if (playerCount > 0)
	{
		...
		for (i = 0; i < playerCount; i++)
		{
			...
			//ƒ	We only copy the first three fields, since the last two are redundant with
			//ƒ	the group info sent next
			machine_move_data((NMUInt8 *) playerInfo, (NMUInt8 *) pip, kJoinApprovedPlayerInfoSize);

			//ƒ	Increment the pointer into our message buffer
	...
	//ƒ	Decrement the group range for the next player
	mNextPlayersGroupStartRange -= 1024;
...
//ƒ	This is quirky, but since we never created a "Real" endpoint for this person,
//ƒ	We need to pass the message to the endpoint here
NMErr
NSpGameMaster::SendJoinDenied(CEndpoint *inEndpoint,  void *inCookie, const NMUInt8 *inMessage)
{
	...
	theReply.header.to = 0;				//ƒ	This person has no honor (or playerID)
...
{
	//ƒ	We need to forward this on to whoever is supposed to get it, possibly including us
	NSpMessageHeader	*theMessage = inERObject->PeekNetMessage();
		
	//ƒ	Any system event should be handled immediately
	...
	NMBoolean	doForward = false;	
		...
		if (doForward)
			ForwardMessage(theMessage);		//ƒ	Forward the message to all recipients, except the sender
		...
		//ƒ	Allow the user to have a crack at it, if he installed a message handler
		//ƒ	User function returns a boolean telling whether or not to enqueue the message
...
NSpGameMaster::PrepareForDeletion(NSpFlags inFlags)
{
	...
	else if (inFlags & kNSpGameFlag_ForceTerminateGame)	//ƒ	They want us to quit even if we can't find another host
	{
	NSpGameTerminatedMessage	message;

		//ƒ	Send our termination
		...
		//ƒ	Tell everyone the game is over
		...
		//ƒ	Delete our advertising endpoint
		if (mAdvertisingATEndpoint)
		{
...
{	
	...
	//ƒ	Ask for someone else to volunteer
...
NSpGameMaster::SendPauseGame()
{
	...
	//ƒ	status = SendUserMessage(&message, kNSpSendFlag_Registered);
...
NSpGameMaster::RemovePlayer(NSpPlayerID inPlayer, NMBoolean inDisconnect)
{
	...
	//ƒ	If we're removing all players, remove all groups, too
	if (removeAll)
	{
		status = DoDeleteGroup(kNSpAllGroups);
	}
	else if (mGameInfo.currentGroups > 0)	//ƒ	Remove this player from any groups
	{
		while (groupIter.Next(&theItem))
		{
			theGroup = (GroupListItem *)theItem;
...
NSpGameMaster::HandleEndpointDisconnected(CEndpoint *inEndpoint)
{
	...
	//ƒ	Find out which player it was
	while (!found && iter.Next(&theItem))
	{
		thePlayer = (PlayerListItem *)theItem;
	...
	//ƒ	First remove this person from our list
	RemovePlayer(thePlayerID, false);

	//ƒ	Notify everyone that this player has left the game
	NSpClearMessageHeader(&message.header);
...
NSpGameMaster::SendJoinRequest(NMConstStr31Param inPlayerName, NSpPlayerType inType)
{
	...
	theMessage.customDataLen = kCustomLocalJoinRequest;			//ƒ	Signifying a local join request
...
NSpGameMaster::ForceRemovePlayer(const NSpPlayerID inPlayerID)
{
	...
	while (!found && iter.Next(&theItem))
	{
		...
		NSpPlayerLeftMessage	message;
			...
			//ƒ	Then notify everyone that this player has left the game
			NSpClearMessageHeader(&message.header);
