...
NSpInitialize(NMUInt32 inStandardMessageSize, NMUInt32 inBufferSize, NMUInt32 inQElements, NSpGameID inGameID, NMUInt32 inTimeout)
{
	...
	//ƒ	We shan't be initialized more than once per application
	if (gPPInitialized)
	...
	//ƒ	Make sure that Mac OS 8.1 or later is installed
	...
	//ƒ	Set the standard message size
	...
	//ƒ	Make a new list for all the games we create in this app
...
NSpProtocol_Dispose(NSpProtocolReference inProtocolRef)
{
	...
	//ƒ	Bad!  The established NetSprocket interface doesn't allow for us
...
NSpProtocol_CreateAppleTalk(NMConstStr31Param inNBPName, NMConstStr31Param inNBPType, NMUInt32 inMaxRTT, NMUInt32 inMinThruput)
{
	...
	//ƒ	Fix bug 1379460, that allows you to register without specifying an nbp name
		...
		NSpFlags					inFlags)
{
	...
	*outGame = (NSpGameReference) theGame;
	...
	//ƒ	We need to keep track of these game objects.  Create a new q element.  Do this now since it might fail
	...
	//ƒ	If the user has installed a join request handler, add it now
	if (gJoinRequestHandler)
		status = master->InstallJoinRequestHandler(gJoinRequestHandler, gJoinRequestContext);
	if (status)
		goto error;

	//ƒ	If the user has installed an async message handler, add it now
	if (gAsyncMessageHandler)
		master->InstallAsyncMessageHandler(gAsyncMessageHandler, gAsyncMessageContext);
	
	//ƒ	If the user has installed a callback handler, add it now
	if (gCallbackHandler)
		master->InstallCallbackHandler(gCallbackHandler, gCallbackContext);
	
	//ƒ	Find out how many protocols we're going to be advertising on
	count = theList->GetCount();

	//ƒ	Confirm that we want to advertise on SOMETHING
	if (count == 0)
	{
...
*/
	...
	//ƒ	We've successfully created the game object.  Store it in our list
	gGameList->Append(theMember);


	//ƒ	Now tell them to add the local player (if any)
...
//ƒ	This feature was never truly implemented in NetSprocket to begin with, so until we hook up
		...
		NSpFlags			inFlags)
{
	...
	//ƒ	Create the private game object
	theGame = new NSpGamePrivate();
	if (theGame == NULL){
		status = kNSpMemAllocationErr;
		goto error;
	...
	//ƒ	Create a new game object with only null initialization
	...
	*outGame = (NSpGameReference) theGame;
	...
	//ƒ	Insert the pointer to the game into our list
	theMember = new uintptrtListMember( (uintptr_t) theGame);
	if (theMember == NULL){
		status = kNSpMemAllocationErr;
		goto error;
	...
	//ƒ	Tell the game object to join the specified game
	...
	info->maxPlayers = 0;
	info->topology = kNSpClientServer;		//ƒ HACK!!!!
		...
		NSpPlayerType 		inNewType)
{
	...
	//ƒ	Make sure that this is the server game reference
	server = theGame->GetMaster();
	
	if (NULL == server)
		return (kNSpInvalidGameRefErr);

	//ƒ	Ok, we've got a server game object, ask it to apply the change
...
NSpPlayer_Remove(NSpGameReference inGame, NSpPlayerID inPlayerID)
{
	...
	//ƒ	Ok, we've got a server game object, ask it to remove the player
...
NSpGetVersion(void)
{
	...
	//ƒ	Note that we do nothing fancy to obtain the version if this is a non-Mac platform.
	...
	void				**outAddress)
{
	...
	//ƒ	Ok, our setup is kosher.  Jump into the server
	//ƒ	game object and free the address
	...
	char				**outAddress)
{
	...
	//ƒ	Ok, our setup is kosher.  Jump into the server
	//ƒ	game object and get the address
...
NSpConvertOTAddrToAddressReference(OTAddress *inAddress)
{
	...
	//ƒ	Determine whether the OTAddress is an AppleTalk address or an IP address and
