		...
		NSpFlags		inFlags)
{
	...
	if (inGameName == NULL)
	{
		//ƒ	Use "untitled game" if we get a NULL
//		GetIndString(mGameInfo.name, kNSp_STRn_GameClassStrings, kUntitledGame);
	...
	//ƒ	Set up the password for entry into the game
	if (inPassword)
	...
	//ƒ	Lets make a bunch of event records, and add them to the free q
	mFreeQ = new NMLIFO();
	op_assert(mFreeQ);
	mFreeQ->Init();
...
NSpGame::FreeNetMessage(NSpMessageHeader *inMessage)
{
	...
	//ƒ	If it's a big message, we alloced it, so just free it
	...
	//ƒ	It's a standard sized message.  Get a cookie to reuse it
	...
	//ƒ	Release the message back to the free q
...
NSpGame::NSpPlayer_GetInfo(NSpPlayerID inID, NSpPlayerInfoPtr *outInfo)
{
	...
	//ƒ	First find the player in the list
	...
	//ƒ	Our player list doesn't really contain the groups.  We need to look through
	//ƒ	the group list and determine which groups we belong to.
	//ƒ	This could be improved.
	FillInGroups(inID, &groups, &groupCount);

	//ƒ	Otherwise, copy the info
...
NSpGame::NSpPlayer_GetEnumeration(NSpPlayerEnumerationPtr *outPlayers)
{
	...
	//ƒ	Allocate enough for the enumeration
	...
	//ƒ	We're going to increment this as we add, in case we have an allocation problem
	thePlayers->count = 0;
	...
	while (iter.Next(&theItem))
	{
		...
		//ƒ	This will allocate and fill in the info
...
NSpGame::NSpPlayer_ReleaseEnumeration(NSpPlayerEnumerationPtr inPlayers)
{
	...
	//ƒ	NSpMessage_Release all the player info structures
	for (NMSInt32 i = 0; i < count; i++)
	{
		NSpPlayer_ReleaseInfo(inPlayers->playerInfo[i]);
	}

	//ƒ	Then release the players structure itself
...
NSpGame::NSpGroup_GetInfo(NSpGroupID inGroupID, NSpGroupInfoPtr *outInfo)
{
	...
	//ƒ	Otherwise, allocate enough for all the player IDs
	...
	//ƒ	Fill in the info with the play IDs
...
//ƒ	This assigns a group id locally, and tells everyone else
//ƒ	Maybe it should ask the server for a group ID
NMErr
NSpGame::NSpGroup_Create(NSpGroupID *outGroupID)
{
	...
	id = mNextAvailableGroupID--;	//ƒ	Group IDs go down as you 'increment'

	//ƒ	Send a message to the server, requesting a new group
	...
	//ƒ	Handle this immediately, since the user may try to use this
	//ƒ	group ID as soon as she creates it
...
//ƒ	Send a message to everyone, telling them to delete the group
NMErr
NSpGame::NSpGroup_Delete(NSpGroupID inGroupID)
{
	...
	//ƒ	Send a message to everyone, including ourselves, telling them to make a new group
	NSpClearMessageHeader(&message.header);
...
NSpGame::NSpGroup_AddPlayer(NSpGroupID inGroupID, NSpPlayerID inPlayerID)
{
	...
	//ƒ	Send a message to all, telling them to create this new group
	status = SendUserMessage(&message.header, kNSpSendFlag_Registered);
...
//ƒ	This is an O(n^2) routine.  Blegh!
void
NSpGame::FillInGroups(NSpPlayerID inPlayer, NSpGroupID **outGroups, NMUInt32 *outGroupCount)
{
	...
	//ƒ	We don't know how many groups this player is a member of until we iterate
	//ƒ	Through the list.  Therefore, alloc enough for all the groups
...
NSpGame::HandleCreateGroupMessage(TCreateGroupMessage *inMessage)
{
	...
	//ƒ	find the group in the list
	while ((false == found) && iter.Next(&theItem))
	{
		theGroup = (GroupListItem *)theItem;

		//ƒ	The group already exists, just ignore the message and move on
	...
	//ƒ	Pass the message up to the client
...
NSpGame::HandleAddPlayerToGroupMessage(TAddPlayerToGroupMessage *inMessage)
{
	...
	//Try_
	{
		...
		//ƒ	We found it.  Let's add the player
...
NSpGame::DoSelfSend(NSpMessageHeader *inHeader, void *inBody, NSpFlags inFlags, NMBoolean inCopy)
{
	...
	//ƒ	Message headers and their data are now contiguous in memory, so
...
NSpGame::HandleEventForSelf(ERObject *inERObject, CEndpoint *inEndpoint)
{
	...
	//ƒ	Set the endpoint and address
	if (inEndpoint)
...
NSpGame::NSpMessage_Get(NSpMessageHeader **outMessage)
{
	...
	//ƒ	If there aren't already messages in our list, dump the q
	if (!mPendingMessages)
	{
		if (mEventQ->IsEmpty())
			return false;
	...
	//ƒ	This eats out the nice creme filling and leaves only the cookie
	theMessage = theERObject->RemoveNetMessage();
	*outMessage = theMessage;

	//ƒ	Put the ERObject in the cookie q, waiting for a new NSpMessageHeader before going back to the free q
