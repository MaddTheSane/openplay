...
PlayerListItem::~PlayerListItem()
{
	//ƒ	We delete the info but not the endpoint.  Someone else might own it.
	if (NULL != info)
...
GroupListItem::AddPlayer(PlayerListItem *inPlayer)
{
	...
	//ƒ	We can't just add this item, since it belongs to another list
	//ƒ	Instead add an item that contains a pointer to it
...
GroupListItem::RemovePlayer(NSpPlayerID inPlayer)
{
	...
	//ƒ	We did find it.  Let's remove it
...
SendQItem::SendQItem(void *inData, NMUInt32 inBytesSent)
{
	...
	//ƒ	Initialize everything to something boring, just in case.
	...
	//ƒ	Now fill them in right.
	
	//ƒ	If this is a non-Macintosh platform, then every path to this point
