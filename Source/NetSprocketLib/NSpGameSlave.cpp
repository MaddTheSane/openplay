...
*/
	...
	if (kNMNoError == err)
	{
		//ƒ	Initialize our endpoint for send/receive only
		status = mEndpoint->InitNonAdvertiser( (NSpProtocolPriv *) inAddress);
		//ThrowIfOSErr_(err);
		if (status)
			goto error;
		
		//ƒ	Send our join request
...
NSpGameSlave::SendUserMessage(NSpMessageHeader *inMessage, NSpFlags inFlags)
{
	...
	//ƒ	If it's just to ourselves, handle that special case
	if (inMessage->to == mPlayerID && !bSelfSent)
	{
		status = DoSelfSend(inMessage, (NMUInt8 *)inMessage + sizeof (NSpMessageHeader), inFlags);
		bSelfSent = true;
	}
	else if (inMessage->to < kNSpMasterEndpointID)	//ƒ	We need to handle if its a group. In case we're a member
	{
		if (!bSelfSent)
		{
...
NSpGameSlave::HandleJoinApproved(TJoinApprovedMessagePrivate *inMessage, NMUInt32 inTimeReceived)
{
	...
	//ƒ	Get the info for the current players
	...
	NMUInt8				*q = (NMUInt8 *) p;
		...
		for (i = 0; i < inMessage->playerCount; i++)
		{
			...
			//ƒ	We always get to other players via the server
			//ƒ	ƒ This should change if we do client-client topology or fault-tolerance
	...
	if (handled)
	{
		...
		//ƒ	The to field contains our player id
		mPlayerID = inMessage->header.to;

		//ƒ	Set the timestamp differential
...
NSpGameSlave::HandlePlayerJoined(NSpPlayerJoinedMessage *inMessage)
{
	...
	//ƒ	If it wasn't us that joined, add them
...
NSpGameSlave::PrepareForDeletion(NSpFlags inFlags)
{
	...
	if (mGameState == kStopped)
	{
		//ƒ	Remove all the players, in case we never did get the disconnect
		RemovePlayer(kNSpAllPlayers, false);

		//ƒ	Wait for the server to disconnect us
	...
	{
		//ƒ	Do an orderly disconnect
		status = mEndpoint->Disconnect(true);

		//ƒ	Wait for the server to ack it
//		status = mEndpoint->WaitForDisconnect(5);
...
NSpGameSlave::ProcessSystemMessage(NSpMessageHeader *inMessage)
{
	...
	switch (inMessage->what)
	{
		...
		//ƒ	Message type handling of kNSpPlayerTypeChanged added here
...
NSpGameSlave::RemovePlayer(NSpPlayerID inPlayer, NMBoolean inDisconnect)
{
	...
	//ƒ	Remove the player from any groups that they are in

	//ƒ	In theory we should look at the player's group list
	//ƒ	and just remove them from those.  However, we don't have
	//ƒ	any clean method for that except for sending ourselves a
	//ƒ	RemovePlayerFromGroup message and that has a lot of overhead.
	while (groupIter.Next(&theItem))
	{
	GroupListItem	*theGroup;
	...
	//ƒ	Then remove them from the list of players
...
NSpGameSlave::HandleEndpointDisconnected(CEndpoint *inEndpoint)
{
	...
	{
		//ƒ	Send a local message that the game has been terminated
		NSpClearMessageHeader(&message.header);
		...
		//ƒ	Tell the user that the game is over
		status = DoSelfSend(&message.header, NULL, kNSpSendFlag_Registered);
	}

	//ƒ	Remove all the players
	RemovePlayer(kNSpAllPlayers, false);

	//ƒ	Receiving this message means we've been disconnected from the server
	mGameState = kStopped;
